...
resource "signalfx_detector" "container_ready" {
  name = "${join("", formatlist("[%s]", var.prefixes))}[${var.environment}] Kubernetes container status"
...
EOF
  ...
  rule {
    description           = "has not passed its readiness probe"
    severity              = "Warning"
    detect_label          = "WARN"
    disabled              = coalesce(var.container_ready_disabled, var.detectors_disabled)
    notifications         = coalescelist(var.container_ready_notifications, var.notifications)
    parameterized_subject = "[{{ruleSeverity}}]{{{detectorName}}} {{{readableRule}}} ({{inputs.signal.value}}) on {{{dimensions}}}"
  }
}
...
resource "signalfx_detector" "deployment_crashloopbackoff" {
  ...
  program_text = <<-EOF
    signal = data('kubernetes.container_restart_count', filter=filter('container_status', 'waiting') and filter('deployment', '*') and filter('container_status_reason', 'CrashLoopBackOff') and ${module.filter-tags.filter_custom})${var.deployment_crashloopbackoff_aggregation_function}${var.deployment_crashloopbackoff_transformation_function}.publish('signal')
...
resource "signalfx_detector" "daemonset_crashloopbackoff" {
  ...
  program_text = <<-EOF
    signal = data('kubernetes.container_restart_count', filter=filter('container_status', 'waiting') and filter('daemonSet', '*') and filter('container_status_reason', 'CrashLoopBackOff') and ${module.filter-tags.filter_custom})${var.daemonset_crashloopbackoff_aggregation_function}${var.daemonset_crashloopbackoff_transformation_function}.publish('signal')
...
resource "signalfx_detector" "job_failed" {
  name = "${join("", formatlist("[%s]", var.prefixes))}[${var.environment}] Kubernetes job failed"

  program_text = <<-EOF
    signal = data('kubernetes.job.failed', filter=${module.filter-tags.filter_custom})${var.job_failed_aggregation_function}${var.job_failed_transformation_function}.publish('signal')
